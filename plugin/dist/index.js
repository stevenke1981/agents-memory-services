...
exports.shouldCaptureTurn = void 0;
exports.shouldRecall = shouldRecall;
exports.capturePriority = capturePriority;
...
const integerEnv = (name, fallback, min, max) => Math.round(numberEnv(name, fallback, min, max));
...
const config = {
    ...
    consolidationInterval: integerEnv("AMS_CONSOLIDATION_INTERVAL_MS", 21600000, 0, 604800000),
};
const LOW_SIGNAL = /^(?:hi|hello|hey|thanks|thank you|ok(?:ay)?|continue|好的?|謝謝|嗨|哈囉|在嗎|繼續|收到|嗯+|喔+)[\s!！?？。.]*$/iu;
const EXPLICIT_MEMORY = /(?:remember|from now on|always|never|prefer|preference|my default|keep in mind|記住|記得|從現在|以後都|一律|永遠|偏好|我的預設|不要忘記)/iu;
const DURABLE = /(?:decid(?:e|ed|ing)|chose|implemented|added|removed|renamed|migrat(?:e|ed|ion)|architecture|workflow|convention|pattern|root cause|fix(?:ed|ing)?|resolved|regression|deploy(?:ed|ment)?|release|database|schema|repository|project|configuration|benchmark|test(?:s)? pass|決定|選擇|實作|新增|移除|重新命名|遷移|架構|流程|慣例|模式|根本原因|修復|解決|回歸|部署|發布|資料庫|結構|儲存庫|專案|設定|基準|測試通過)/iu;
const TECHNICAL = /(?:\b(?:rust|typescript|javascript|python|swift|cargo|npm|mcp|api|sql|sqlite|tantivy|usearch|llama\.cpp|embedding|token|branch|commit|pull request|ci)\b|[\\/][\w.-]+|\.[a-z0-9]{1,8}\b|錯誤|例外|編譯|分支|提交|記憶|檢索)/iu;
const EPHEMERAL = /(?:just for now|temporary|temporarily|try this|maybe|perhaps|for this test only|先試試|暫時|臨時|也許|可能|只用於測試)/iu;
const GLOBAL_PREFERENCE = /(?:from now on|always|for every project|across projects|my default|my preference|globally|以後都|從現在|所有專案|全部專案|我的偏好|預設都|一律|永遠)/iu;
let activeCalls = 0;
let failures = 0;
let disabledUntil = 0;
...
function capturePriority(userMessage, assistantMessage) {
    if (!config.enabled)
        return 0;
    const user = normalized(userMessage);
    const assistant = normalized(assistantMessage);
    const length = user.length + assistant.length;
    if (length < config.minCaptureChars || (LOW_SIGNAL.test(user) && assistant.length < 200))
        return 0;
    if (EXPLICIT_MEMORY.test(user))
        return 3;
    let score = 0;
    if (DURABLE.test(user))
        score += 2;
    if (DURABLE.test(assistant))
        score += 1;
    if (TECHNICAL.test(user))
        score += 1;
    if (length >= 700)
        score += 1;
    if (length >= 1800)
        score += 1;
    if (EPHEMERAL.test(user) && !DURABLE.test(assistant))
        score -= 1;
    return score >= 3 ? 3 : score >= 2 ? 2 : score >= 1 && length >= 700 ? 1 : 0;
}
const shouldCaptureTurn = (user, assistant) => capturePriority(user, assistant) > 0;
...
function buildCapture(userMessage, assistantMessage) {
...
const rank = (memory) => (memory.score_final ?? 0) * 0.8 + memory.importance_score * 0.2;
function selectMemories(memories) {
    const unique = new Map();
    for (const memory of memories) {
        const score = memory.score_final ?? 0;
        if (score < config.minRecallScore &&
...
async function callOutcome(client, tool, params, milliseconds) {
    ...
    void operation.then(() => { activeCalls = Math.max(0, activeCalls - 1); failures = 0; }, () => { activeCalls = Math.max(0, activeCalls - 1); });
    try {
...
async function searchScope(ctx, query, scope, topK) {
    ...
    if (running)
        return running;
    const pending = (async () => {
        const result = await safeCall(ctx.mcp, "search_memories", {
            query,
            top_k: topK,
            scope,
        ...
        const memories = result === undefined ? [] : (0, response_1.parseMemoriesResponse)(result);
        if (result !== undefined)
            cache(key, memories);
        return memories;
...
const sameGroup = (a, b) => a.client === b.client && a.sessionId === b.sessionId && a.scope === b.scope && a.projectId === b.projectId;
function scheduleDrain(delay = config.debounceMs) {
    if (drainTimer !== undefined || drainPromise !== undefined)
        return;
...
function takeBatch() {
    ...
    for (let index = 0; index < queue.length && batch.length < config.batchSize;) {
        if (sameGroup(first, queue[index]))
    ...
    drainPromise = (async () => {
        while (queue.length) {
            ...
            if (!batch.length)
                break;
            const first = batch[0];
            const outcome = await callOutcome(first.client, "add_memory", {
...
function shouldConsolidate() {
    if (!config.consolidationInterval || Date.now() - lastConsolidation < config.consolidationInterval)
        return false;
    lastConsolidation = Date.now();
    return true;
}
function detached(task) {
...
exports.default = {
    ...
    hooks: {
        onChatStart: async (ctx) => {
            ...
            if (ctx.projectId)
                searches.unshift(searchScope(ctx, query, "Project", config.recallTopK));
        ...
        onSessionEnd: async (ctx) => {
            detached(async () => {
                ...
                if (shouldConsolidate()) {
                    await safeCall(ctx.mcp, "consolidate_memories", { scope: ctx.projectId ? "Project" : "Global", project_id: ctx.projectId }, config.consolidationTimeout);
                }
            });
        },
    },
